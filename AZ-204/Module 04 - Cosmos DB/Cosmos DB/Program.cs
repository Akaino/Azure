    ...
    class Program
    {
        ...
        */
        private async Task AddItemsToContainerAsync()
        {
            ...
            {
                Id = "Andersen.1", // UNIQUE ID - Remove key for auto-generation
                LastName = "Andersen",
                Parents = new Parent[]
                {
            ...
            var andersenFamilyResponse = await this.container.ReadItemStreamAsync(andersenFamily.Id, new PartitionKey(andersenFamily.LastName));
            
            if (andersenFamilyResponse.StatusCode == HttpStatusCode.NotFound)
            {
                // Create an item in the container representing the Andersen family. Note we provide the value of the partition key for this item, which is "Andersen"
                var item = await this.container.CreateItemAsync(andersenFamily, new PartitionKey(andersenFamily.LastName));
            ...
            ResponseMessage wakefieldFamilyResponse = await this.container.ReadItemStreamAsync(wakefieldFamily.Id, new PartitionKey(wakefieldFamily.LastName));

            if (wakefieldFamilyResponse.StatusCode == HttpStatusCode.NotFound)
            {
                // Create an item in the container representing the Wakefield family. Note we provide the value of the partition key for this item, which is "Wakefield"
                var wakefieldFamilyResponseItem = await this.container.CreateItemAsync<Family>(wakefieldFamily, new PartitionKey(wakefieldFamily.LastName));

                // Note that after creating the item, we can access the body of the item with the Resource property off the ItemResponse. We can also access the RequestCharge property to see the amount of RUs consumed on this request.
                Console.WriteLine("Created item in database with id: {0} Operation consumed {1} RUs.\n", wakefieldFamilyResponseItem.Resource.Id, wakefieldFamilyResponseItem.RequestCharge);
            }
            else
            {
