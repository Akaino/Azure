...
namespace Demo_04___Batch
{
    class Program
    {
        ...
        static String batchAccountName = azureConfig.GetValue("BatchAccountName").ToString();
        static String batchAccountKey = azureConfig.GetValue("BatchAccountKey").ToString();
        static String batchAccountUri = azureConfig.GetValue("BatchAccountUri").ToString();
        ...
        // pool information
        static string PoolId = "WinFFmpegPool";
        static int DedicatedNodeCount = 0;
        static int LowPriorityNodeCount = 5;
        static string PoolVMSize = "STANDARD_A1_v2";
        static string JobId = "WinFFmpegJob";
        
        static string appPackageId = "ffmpeg";
        static string appPackageVersion = "3.4";
        static void Main(string[] args)
        {
            ...
            if (String.IsNullOrEmpty(batchAccountName) ||
                ...
                String.IsNullOrEmpty(batchAccountUri) ||
        ...
        private static async Task MainAsync()
        {
            ...
            //do
            //{
            //    var response = await blobClient.GetContainerReference(inputContainerName).ListBlobsSegmentedAsync(continuationToken);
            ...
            //    {
            //        ResourceFile file = ResourceFile.FromStorageContainerUrl(results[0].StorageUri.PrimaryUri.ToString());
            //        inputFiles.Add(file);
            //    }
            //}
            //else
            //{
                inputFiles = await UploadFilesToContainerAsync(blobClient, inputContainerName, inputFilePaths);
            //}


            foreach (var item in inputFiles)
            {
                Console.WriteLine("File including SAS: {0}",item.StorageContainerUrl);
            }

            // Obtain a shared access signature that provides write access to the output container to which
            // the tasks will upload their output.

            string outputContainerSasUrl = GetContainerSasUrl(blobClient, outputContainerName, 
            ...
            BatchSharedKeyCredentials sharedKeyCredentials = new BatchSharedKeyCredentials(batchAccountUri, batchAccountName, batchAccountKey);
        ...
        private static async Task<ResourceFile> UploadResourceFileToContainerAsync(CloudBlobClient blobClient, string containerName, string filePath)
        {
            ...
            if (await blobData.ExistsAsync())
            {
                Console.WriteLine("Skipping file {0}. File already exists.", filePath);
            } else
            {
        ...
        private static async Task CreatePoolIfNotExistAsync(BatchClient batchClient, string poolId)
        {
            ...
            try
            {
                ...
                Console.WriteLine(pool.ApplicationPackageReferences);
            }
        ...
        private static async Task<List<CloudTask>> AddTasksAsync(BatchClient batchClient, string jobId, List<ResourceFile> inputFiles, string outputContainerSasUrl)
        {
            ...
            for (int i = 0; i < inputFiles.Count; i++)
            {
                ...
                /// subscriptions/dd6f01f9-0e91-4f72-869d-51a5268bd92a/resourcegroups/az203/providers/microsoft.batch/batchaccounts/az203batchacc/applications/ffmpeg
        ...
        private static async Task<bool> MonitorTasks(BatchClient batchClient, string jobId, TimeSpan timeout)
        {
            ...
            catch (TimeoutException e)
            {
                ...
                Console.WriteLine(batchClient.JobOperations.GetJob(jobId).ExecutionInformation);
                Console.WriteLine(batchClient.JobOperations.GetJob(jobId).Statistics);
